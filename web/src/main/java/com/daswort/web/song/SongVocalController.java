...
public class SongVocalController {
    ...
    @GetMapping
    public ResponseEntity<List<Vocal>> findAll() {
    ...
    public ResponseEntity<?> removeVocal(@PathVariable("vocalId") String vocalId) {
        vocalRepository.delete(vocalId);
        return ResponseEntity.ok().build();
    }

    @PostMapping
    public ResponseEntity<?> saveVocal(@RequestBody @Valid IdTitleDto topic) {
        vocalRepository.save(topic.getId(), topic.getTitle());
        return ResponseEntity.ok().build();
    }
}
